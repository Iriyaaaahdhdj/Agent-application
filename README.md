...
为验证课程资料问答助手在不同学习场景下的可用性，项目补充了评测样例、badcase 分类和自动化统计脚本，用于持续观察回答质量、格式稳定性和资料边界处理能力。

- `data/eval_cases.csv`：30 条自建测试样例，包含用户问题、预期意图、必须命中点、禁止行为、人工评分、格式通过率、拒答表现和耗时记录。
- `scripts/evaluate_cases.py`：自动统计不同 Prompt 版本、任务类型和资料覆盖范围下的平均分、4 分及以上占比、依据充分性、格式通过率、拒答通过率和 badcase 类型。
- `docs/大模型评测报告.md`：记录评测设计、字段口径、Prompt 迭代和后续优化 Backlog。
- `docs/大模型评测结果汇总.md`：由脚本生成的评测指标汇总，便于复盘不同版本的表现。
...
如果只是检查核心函数是否正常，不需要配置 API Key，可以运行：

```bash
python scripts/smoke_test.py
```

该脚本会验证文本清洗、资料读取、模型输出归一化和 Markdown 报告生成等核心逻辑。
